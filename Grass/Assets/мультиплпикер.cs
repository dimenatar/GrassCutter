...
public class мультиплпикер : MonoBehaviour {
    ...
    public static мультиплпикер activeColorPicker = null;
    ...
    void FindGrass()
    {
        if (!find)
        {
            receivers = GameObject.FindGameObjectsWithTag("grass");
            Debug.Log(receivers.Length + "Колво травы");
            for (int i = 0; i < 6400; i++)
            {
                try
                {
                    //receivers[i].SendMessage(colorGetFunctionName, this, SendMessageOptions.DontRequireReceiver);
                }
        ...
        if (mState == ESTATE.Showed)
        {
            ...
            if (GUI.Button(new Rect(startPos.x + sizeCurr + 10, startPos.y + 150, 60, 20), "Apply"))
            {
                ...
                SelectedColor = TempColor;
                    ...
                    try
                    {
                        receivers[i].SendMessage(colorSetFunctionName, SelectedColor, SendMessageOptions.DontRequireReceiver);
                    }
                    catch (System.Exception) { }
    ...
    public void ApplyColor()
    {
        ...
        { 
            try
            {
                Debug.Log("Я хороший мультипликер я поменял цвет");
                receivers[i].SendMessage(colorSetFunctionName, SelectedColor, SendMessageOptions.DontRequireReceiver);
            }
            catch (System.Exception) { }
        }
